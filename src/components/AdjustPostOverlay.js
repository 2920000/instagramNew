...
function AdjustPostOverlay() {
    ...
    const handleDeletePost=()=>{
     dispatch(actionHandleDeletePost(isAjustPost.docIdToDelete,isAjustPost.docIdToUpdateNumberPosts,isAjustPost.prePostsNumberOfUser))
     dispatch(ADJUST_POST({
      isShowOverlay:false,
    }))
    if(url!=='/')
 ...
 <div className="fixed right-0  top-0 bottom-0 left-0 mix-blend-multiply bg-[#000000A6] z-50"></div>
  <div  ref={adjustPostBoxRef}  className="fixed right-0 top-0 bottom-0 left-0 flex justify-center items-center z-50 ">
           <ul className="w-[410px] rounded-xl animate-scale list-none bg-whiteColor">
                  ...
                  :<li className="py-3.5 text-center text-sm font-bold cursor-pointer border-b border-borderColor text-[#ED4956] active:bg-greyLightColor rounded-t-xl">Báo cáo</li>}
                    ...
                    <li className="py-3.5 text-center text-sm border-b cursor-pointer border-borderColor active:bg-greyLightColor">Nhúng</li>
                    <li onClick={handleOffBox}  className="py-3.5 text-center text-sm cursor-pointer rounded-xl active:bg-greyLightColor">Hủy</li>
