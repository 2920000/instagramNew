 ...
 if(messageById.docId){
    ...
    return (
        <div className='flex flex-col w-full'>
            ...
            <div id='scroll' className='grow overflow-y-auto p-5 
            ' >
            ...
            <form className='px-8 py-5 flex relative items-center  w-full'>
               <div className=' flex relative items-center border px-2 border-borderColor rounded-[35px]  w-full '>
                <span className='p-2'><FiSmile className=' left-[50px]  text-2xl'/></span>
                <input value={input} onChange={(e)=>{setInput(e.target.value)}} className='p-2.5  outline-none w-full bg-[#fafafa] text-sm' placeholder='Nhắn tin...' />
                <button className='text-sm disabled:hidden  font-medium text-textFollowColor p-2 cursor-pointer  ' disabled={!input} onClick={(e)=>{handleSubmitMessage(e,filterUser1)}} >Gửi</button>
               </div>
            </form>
        </div>
      )
