...
function SearchUser({
  ...
  const filterGetUserName = allUsersExceptLoginUser.filter((user) => {
    ...
    const newStrInput = inputNoWhiteSpace
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    const newStrUserName = userNameNoWhiteSpace
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
  ...
  return (
    <>
      {searchBoxstatus && (
        <div className="absolute flex flex-col  w-[360px] h-[350px] rounded-md top-[48.5px] shadow-lg left-[-60px] bg-whiteColor ">
          <div className="absolute h-[10px] w-[10px] rotate-45 shadow-[0_0_1px_grey] z-[-1] bg-whiteColor  top-[-5px] left-2/4"></div>
          <div className="grow overflow-y-auto">
            {input ? (
              <div className="pt-2">
                {filterGetUserName.map((user) => (
                  <Link
                    to={`/${user.userId}`}
                    onClick={handleCloseSearchBox}
                    className="flex  gap-x-2 items-center p-2 hover:bg-greyLightColor cursor-pointer"
                    key={user.userId}
                  >
                    <img
