...
export const getUsersFromFirebase = (loginUserId) => async (dispatch) => {
  ...
  onSnapshot(collection(db, "users"), (usersDocs) => {
    ...
    const getLoginUserFullData = allUsers.find(
      (user) => user.userId === loginUserId
    );
    dispatch(GET_LOGIN_USER_FULL_DATA(getLoginUserFullData));
    const getUsersExceptLoginUser = allUsers.filter(
      (user) => user.userId !== loginUserId
    );
...
export const {
  GET_ALL_USERS,
  GET_LOGIN_USER_FULL_DATA,
  GET_USERS_EXCEPT_LOGIN_USER,
} = usersSlice.actions;
// selects
export const selectAllUsers = (state) => state.users.allUsers;
export const selectLoginUserFullData = (state) => state.users.loginUserFullData;
export const selectAllUserExceptLoginUser = (state) =>
  state.users.allUsersExceptLoginUser;
// async actions
export const actionUpdateFollow =
  (loginUserFullData, userClickedToFollow) => async () => {
    ...
    const checkFollow = loginUserFullData.following.every(
      (user) => user.userId !== userClickedToFollow.userId
    );
    if (checkFollow) {
      ...
      const arrayContainIndexOfLoginUser = loginUserFullData.following.map(
        (user, index) => {
      ...
      const indexOfUserToUnfollow = arrayContainIndexOfLoginUser.find(
      ...
      const arrayContainIndexOfUserClicked = userClickedToFollow.followers.map(
        (user, index) => {
      ...
      const indexOfUserToDeletefollow = arrayContainIndexOfUserClicked.find(
        (index) => index !== undefined
      );
